let elementoSaudacao = document.querySelector("#idSaudacao");
elementoSaudacao.innerHTML = `Hello, ${user.name}`;
...
axios({
  ...
  let total = response.data[0].SaldoAtual;
...
axios({
  method: "get",
  url: `http://localhost:3000/saldoPrevisto?userId=${user.id}`,
}).then(function (response) {
  let saldoPrevistoDiv = document.querySelector("#saldo-previsto-div");
  let total = response.data[0].SaldoPrevisto;

  saldoPrevistoDiv.innerHTML = `<p>${total}€</p>`;
});
  ...
  response.data.forEach((movimento) => {
    ...
    ultimosMovimentosDiv.innerHTML += `
            <a href="#" data-id="1" onclick="toggleActive(this)" class="list-group-item list-group-item-action" aria-current="true">
                ...
                <p class="mb-1 ${
                  movimento.tipo == 0 ? "text-success" : "text-danger"
                }">${movimento.valor}€</p>
            </a>
            `;
  });
